...
<template>
  <div>
    ...
    <!-- 按 Agent（默认视图） -->
    <div class="space-y-1">
      <span class="text-[10px] text-cockpit-muted">By Agent</span>
      <TokenBarChart :data="barData" />
    </div>

    <!-- 按项目（饼占位，Phase 3 mvp 用条形图代替） -->
    <div class="mt-3 space-y-1">
      <span class="text-[10px] text-cockpit-muted">By Workspace</span>
      ...
      <div v-else class="text-[10px] text-cockpit-muted italic">No workspace data</div>
    </div>

    <!-- 按日期 -->
    <div class="mt-3 space-y-1">
      <span class="text-[10px] text-cockpit-muted">By Date (7 days)</span>
      ...
      <div v-else class="text-[10px] text-cockpit-muted italic">No daily data</div>
    </div>
  </div>
</template>
